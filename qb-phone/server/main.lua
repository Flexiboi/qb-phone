...
local WebHook = 'https://discord.com/api/webhooks/1204908142375346186/m13gtd_AQswbBD_5Llhac_NnIRWEUDdbQtFzUxjrjSeyzQEpa9_4Vkqs1h8a33ox-VkE'
...
local function GenerateOwnerName()
    ...
    local names = {
        [1] = { name = vn[math.random(1,#vn)].. ' ' ..an[math.random(1,#an)], citizenid = cid[math.random(1,#cid)] },
    }
    return names[1]
end
    ...
    if Target ~= nil then
        if Calls[Target.PlayerData.citizenid] ~= nil then
            if Calls[Target.PlayerData.citizenid].inCall then
                NpcData = { IsNpc = false, event = nil}
                cb(false, true, NpcData)
            else
                NpcData = { IsNpc = false, event = nil}
                cb(true, true, NpcData)
            end
        else
            NpcData = { IsNpc = false, event = nil}
            cb(true, true, NpcData)
        ...
        if Config.NpcCalls[ContactData.number] ~= nil then
            NpcData = { IsNpc = true, event = Config.NpcCalls[ContactData.number].event}
            cb(true, true, NpcData)
        else
            NpcData = { IsNpc = false, event = nil}
            cb(false, false, NpcData)
        end
    end
end)

QBCore.Functions.CreateCallback('qb-phone:server:GetCallStateAds', function(_, cb, ContactData)
    local Target = QBCore.Functions.GetPlayerByPhone(tostring(ContactData.number))
    local NpcData = {}
    if Target ~= nil then
        if Calls[Target.PlayerData.citizenid] ~= nil then
            if Calls[Target.PlayerData.citizenid].inCall then
...
QBCore.Functions.CreateCallback('qb-phone:server:GetPhoneData', function(source, cb)
    ...
    if Player ~= nil then
        ...
        -- end
        -- exports.oxmysql:fetch('SELECT * FROM fd_advanced_banking_invoices WHERE recipient = ?',
        --  {Player.PlayerData.citizenid}, function(data)
        ...
        local Adverts = MySQL.query.await('SELECT * FROM phone_adverts WHERE `date` > NOW() - INTERVAL ? hour', { Config.AdsDuration })
        ...
        local notes = MySQL.query.await('SELECT * FROM `phone_notes` WHERE `citizenid` = ?', {Player.PlayerData.citizenid})
        local Notes = {}
        if notes[1] ~= nil then
...
QBCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, amount, invoiceId)
    exports.fd_banking:payInvoice(source, invoiceId, true)
    cb(true)
end)
...
QBCore.Functions.CreateCallback('qb-phone:server:HasPhone', function(source, cb)
    ...
    if Player ~= nil then
        ...
        if HasPhone ~= nil and HasVpn == nil then
            cb(true, false)
        elseif HasPhone == nil and HasVpn ~= nil then
            cb(false, true)
        elseif HasPhone ~= nil and HasVpn ~= nil then
            cb(true, true)
        else
    ...
    if result[1] ~= nil then
        ...
        if Player.Functions.RemoveMoney('bank', amount) then
            if Reciever ~= nil then
                if ((Reciever.PlayerData.job.name == 'ambulance' or Reciever.PlayerData.job.name == ('police' or 'sasp')) and Player.PlayerData.job.onduty) then
                    exports.fd_banking:AddMoney(Reciever.PlayerData.job.name, amount, Player.PlayerData.charinfo.firstname..' '..Player.PlayerData.charinfo.lastname..' betaalde de factuur van €'..amount)
                else
                    Reciever.Functions.AddMoney('bank', amount, 'Geld gekregen van:' .. Player.PlayerData.citizenid)
                end
            else
    ...
    for _, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.job.name == 'lawyer' or Player.PlayerData.job.name == 'realestate' or
                    ...
                    Player.PlayerData.job.name == ('police' or 'sasp') or Player.PlayerData.job.name == 'ambulance' or Player.PlayerData.job.name == 'cluckinbell') and
...
QBCore.Functions.CreateCallback('qb-phone:server:CanDeleteAdvert', function(source, cb, id)
    local src = source
    local Data = MySQL.scalar.await('SELECT citizenid FROM phone_adverts WHERE AdsId = ?', { id })
    local Player = QBCore.Functions.GetPlayer(src)
    if tostring(Data) == tostring(Player.PlayerData.citizenid) then
        cb(true)
    else
        cb(false)
    end
...
RegisterServerEvent('qb-phone:server:AddNote', function(data)
    ...
    if result[#result] ~= nil then
        TriggerClientEvent('qb-phone:client:AddNote', src, { id = result[#result].id, title = data.title, body = data.body})
    end
end)
...
RegisterNetEvent('qb-phone:server:AddAdvert', function(msg, url, cat)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local CitizenId = Player.PlayerData.citizenid
    Adverts[#Adverts+1] = {
    ...
    TriggerClientEvent('qb-phone:client:UpdateAdverts', -1, src, Adverts, Adverts[#Adverts+1], false)
end)

RegisterNetEvent('qb-phone:server:DeleteAdvert', function(id)
    local src = source
    Citizen.Wait(1500)
    for k, v in pairs(AdvertData) do
        if v.AdsId == id then
            MySQL.query.await('DELETE FROM phone_adverts WHERE AdsId = ?', { id })
            table.remove(AdvertData, k)
            TriggerClientEvent('qb-phone:client:UpdateAdverts', -1, src, AdvertData, nil, true)
            break
        end
    end
end)
...
RegisterNetEvent('qb-phone:server:Call911', function(CallId, AnonymousCall)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    for _, v in pairs(QBCore.Functions.GetPlayers()) do
        local Target = QBCore.Functions.GetPlayer(v)
        if Target.PlayerData.job.name == ('police' or 'sasp') and Target.PlayerData.job.onduty and Target.PlayerData.job.grade.level == Config.DispatchJobLevel then
            TriggerClientEvent('qb-phone:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId, AnonymousCall)
            break
        end
    end
...
RegisterNetEvent('qb-phone:server:UpdateTweets', function(NewTweets, TweetData)
    ...
    if Config.Linux then
        MySQL.insert('INSERT INTO phone_tweets (citizenid, firstName, lastName, message, date, url, picture, vpntweet, tweetid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', {
            TweetData.citizenid,
            TweetData.firstName,
            TweetData.lastName,
            TweetData.message,
...
RegisterNetEvent('qb-phone:server:TransferMoney', function(iban, amount)
    ...
    if result[1] ~= nil then
        ...
        if reciever ~= nil then
            local PhoneItem = reciever.Functions.GetItemByName(Config.phoneItem)
        ...
        TriggerClientEvent('QBCore:Notify', src, "Dit accountnummer bestaat niet!", 'error')
    end
end)
...
-- QBCore.Commands.Add('bill', 'Bill A Player', { { name = 'id', help = 'Geluksnummer' }, { name = 'hoeveelheid', help = 'Fine Amount' } }, false, function(source, args)
...
--                     TriggerClientEvent('QBCore:Notify', source, 'Factuur met succes verzonden', 'success')
--                     TriggerClientEvent('QBCore:Notify', billed.PlayerData.source, 'Nieuwe factuur ontvangen')
--                 else
--                     TriggerClientEvent('QBCore:Notify', source, 'Moet een geldig bedrag boven 0 zijn', 'error')
--                 end
--             else
--                 TriggerClientEvent('QBCore:Notify', source, 'Je kunt jezelf niet factureren', 'error')
--             end
--         else
--             TriggerClientEvent('QBCore:Notify', source, 'Burger niet in de stad', 'error')
--         end
--     else
--         TriggerClientEvent('QBCore:Notify', source, 'Geen toegang', 'error')
--     end
-- end)
...
AddEventHandler('qb-phone:server:GiveInvoice', function()
	...
	local Player = QBCore.Functions.GetPlayer(src)
    if Player then
        exports.fd_banking:issueInvoice(source, source, 500, 'Auto laten ophalen/leveren', isSociety)
    end
end)

-- RingTone
RegisterServerEvent('qb-phone:server:playRingtone')
AddEventHandler('qb-phone:server:playRingtone', function(pos, musicId, link, serverId, mp3)
    TriggerClientEvent("qb-phone:client:playRingtone", -1, pos, musicId, link, serverId, mp3)
end)

RegisterServerEvent('qb-phone:server:pauseRingtone')
AddEventHandler('qb-phone:server:pauseRingtone', function(musicId)
    TriggerClientEvent("qb-phone:client:pauseRingtone", -1, musicId)
end)

RegisterServerEvent('qb-phone:server:stopRingtone')
AddEventHandler('qb-phone:server:stopRingtone', function(musicId)
    TriggerClientEvent("qb-phone:client:stopRingtone", -1, musicId)
end)

RegisterServerEvent('qb-phone:server:continueRingtone')
AddEventHandler('qb-phone:server:continueRingtone', function(musicId)
    TriggerClientEvent("qb-phone:client:continueRingtone", -1, musicId)
end)

AddEventHandler('playerDropped', function(source)
    local _source = source
    TriggerClientEvent("qb-phone:client:stopRingtone", -1, tostring(_source))
end)

----- Phone Adverts 

-- RegisterNetEvent('qb-phone:server:DeleteAds', function(AdsId)
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
--     local delete = false
--     local AID = AdsId
--     local Data = MySQL.scalar.await('SELECT citizenid FROM phone_adverts WHERE AdsId = ?', { AID })
--     if tostring(Data) == tostring(Player.PlayerData.citizenid) then
--         MySQL.query.await('DELETE FROM phone_adverts WHERE AdsId = ?', { AID })
--         for k, v in pairs(AdvertData) do
--             if v.AdsId == AID then
--                 table.remove(AdvertData, k)
--                 TriggerClientEvent('qb-phone:client:UpdateAdverts', -1, src, AdvertData, nil, true)
...
RegisterNetEvent('qb-phone:server:UpdateAdverts', function(NewAdverts, AdvertData)
    local src = source
    MySQL.insert('INSERT INTO phone_adverts (citizenid, firstName, lastName, message, date, url, picture, AdsId) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
        AdvertData.citizenid,
        AdvertData.firstName,
        AdvertData.lastName,
        AdvertData.message,
        AdvertData.time,
        AdvertData.url:gsub('[%<>\"()\' $]', ''),
        AdvertData.picture:gsub('[%<>\"()\' $]', ''),
        AdvertData.AdsId
    })
    TriggerClientEvent('qb-phone:client:UpdateAdverts', -1, src, NewAdverts, AdvertData, false)
end)