...
$(document).on('click', '.person-search-result', function(e){
    ...
    var OpenElement = '<div class="person-search-result-name">Naam: '+ClickedPersonData.firstname+' '+ClickedPersonData.lastname+'</div> <div class="person-search-result-bsn">BSN: '+ClickedPersonData.citizenid+'</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Geboortedatum: '+ClickedPersonData.birthdate+'</div> <div class="person-search-result-number">Telefoonnummer: '+ClickedPersonData.phone+'</div> <div class="person-search-result-nationality">Nationaliteit: '+ClickedPersonData.nationality+'</div> <div class="person-search-result-gender">Geslacht: '+Gender+'</div> &nbsp; <div class="person-search-result-warned">Gesignaleerd: '+IsWarrant+'</div> <div class="person-search-result-driverslicense">Rijbewijs: '+HasLicense+'</div>';
...
$(document).on('click', '.person-search-result-apartment > span', function(e){
    ...
    QB.Phone.Notifications.Add("fas fa-university", "MDT", "Huisnummer gekopieerd!", "#badc58", 1750);
...
$(document).on('click', '.person-search-result-house', function(e){
    ...
    var OpenElement = '<div class="person-search-result-name">Eigenaar: '+ClickedHouseData.charinfo.firstname+' '+ClickedHouseData.charinfo.lastname+'</div><div class="person-search-result-bsn">House: '+ClickedHouseData.label+'</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Adres: '+ClickedHouseData.label+' &nbsp; <i class="fas fa-map-marker-alt house-adress-location" id="'+ClickedHouseId+'"></i></div> <div class="person-search-result-number">Laag: '+ClickedHouseData.tier+'</div> <div class="person-search-result-nationality">Garage: ' + GarageLabel + '</div>';
    ...
    if (SearchName !== "") {
        $.post('https://qb-phone/FetchSearchResults', JSON.stringify({
            ...
            if (result != null) {
                ...
                QB.Phone.Notifications.Add("politie", "MDT", "Er zijn geen zoekresultaten!");
                $(".person-search-results").html("");
            }
        });
    } else {
        ...
        $.post('https://qb-phone/FetchVehicleResults', JSON.stringify({
            ...
            if (result != null) {
                ...
                $.each(result, function (i, vehicle) {
                    ...
                    var VehicleElement = '<div class="vehicle-search-result"> <div class="vehicle-search-result-name">'+vehicle.label+'</div> <div class="vehicle-search-result-plate">Nummerplaat: '+vehicle.plate+'</div> <div class="vehicle-opensplit"></div> &nbsp; <div class="vehicle-search-result-owner">Eigenaar: '+vehicle.owner+'</div> &nbsp; <div class="vehicle-search-result-apk">TEGEN: '+APK+'</div> <div class="vehicle-search-result-warrant">Gesignaleerd: '+Flagged+'</div> </div>'
...
$(document).on('click', '.scan-search-vehicle', function(e){
    ...
    $.post('https://qb-phone/FetchVehicleScan', JSON.stringify({}), function(vehicle){
        if (vehicle != null) {
            ...
            QB.Phone.Notifications.Add("politie", "MDT", "Geen voertuig in de buurt!");
...
AddPoliceAlert = function(data) {
    ...
    if (data.alert.coords != undefined && data.alert.coords != null) {
        AlertElement = '<div class="meos-alert" id="alert-'+randId+'"> <span class="meos-alert-new" style="margin-bottom: 1vh;">NIEUW</span> <p class="meos-alert-type">alert: '+data.alert.title+'</p> <p class="meos-alert-description">'+data.alert.description+'</p> <hr> <div class="meos-location-button">PLAATS</div> </div>';
    } else {
        AlertElement = '<div class="meos-alert" id="alert-'+randId+'"> <span class="meos-alert-new" style="margin-bottom: 1vh;">NIEUW</span> <p class="meos-alert-type">alert: '+data.alert.title+'</p> <p class="meos-alert-description">'+data.alert.description+'</p></div>';
    }
...
$(document).on('click', '.meos-recent-alert', function(e){
    ...
    if (alertData != undefined){
        if (alertData.coords != undefined && alertData.coords != null) {
            ...
            QB.Phone.Notifications.Add("politie", "MDT", "Deze waarschuwing heeft geen GPS -locatie!");
        }
    }else {
        QB.Phone.Notifications.Add("politie", "MDT", "Er zijn geen meldingen beschikbaar.");
    }
});
...
$(document).on('click', '.meos-clear-alerts', function(e){
    $(".meos-alerts").html("");
    $(".meos-recent-alerts").html('<div class="meos-recent-alert"> <span class="meos-recent-alert-title">U hebt geen meldingen!</span></div>');
    QB.Phone.Notifications.Add("politie", "MDT", "Alle waarschuwingen zijn verwijderd!");
});
