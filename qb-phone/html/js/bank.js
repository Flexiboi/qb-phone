...
$(document).on('click', '.bank-app-account', function(e){
    ...
    QB.Phone.Notifications.Add("fas fa-university", "Fleeca", "Rekeningnummer.gekopieerd!", "#badc58", 1750);
});
    ...
    if (iban != "" && amount != "") {
            $.post('https://qb-phone/CanTransferMoney', JSON.stringify({
                ...
                if (data.TransferedMoney) {
                    ...
                    QB.Phone.Data.PlayerData.money.bank = data.NewBalance;
                    QB.Phone.Notifications.Add("fas fa-university", "Fleeca", "Je hebt overgeplaatst &#8364; "+amount+"!", "#badc58", 1500);
                } else {
                    QB.Phone.Notifications.Add("fas fa-university", "Fleeca", "Je hebt niet genoeg balans!", "#badc58", 1500);
                }
                QB.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
            });
    } else {
        QB.Phone.Notifications.Add("fas fa-university", "Fleeca", "Vul alle velden in!", "#badc58", 1750);
    }
});
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    $.post('https://qb-phone/PayInvoice', JSON.stringify({
        ...
        if (CanPay) {
            ...
            QB.Phone.Notifications.Add("fas fa-university", "Fleeca", "Jij hebt betaald &#8364;"+InvoiceData.amount+"!", "#badc58", 1500);
            var amountData = $(".bank-app-account-balance").data('balance');
...
QB.Phone.Functions.LoadBankInvoices = function(invoices) {
    if (invoices !== null) {
        ...
        $.each(invoices, function(i, invoice){
            if(invoice.status != 2) {
                var Elem = '<div class="bank-app-invoice" id="invoiceid-'+i+'"> <div class="bank-app-invoice-title"><span style="font-size: 1.3vh; color: white;"><b>Van:</b> '+invoice.issued_by+'</span></div> <div class="bank-app-invoice-description"> <span style="font-size: 1vh; color: white;"><b>Reden:</b> '+invoice.description+'</span></div> <div class="bank-app-invoice-amount">&#8364; '+invoice.amount+'</div> <div class="bank-app-invoice-buttons"> <i class="fas fa-check-circle pay-invoice"></i> </div> </div>';
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        QB.Phone.Notifications.Add("fas fa-university", "Fleeca", "Er is geen bankrekening aan dit nummer verbonden!", "#badc58", 2500);
    }
    QB.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});