                ...
                if (status.CanCall) {
                    if (!status.InCall) {
                        if (QB.Phone.Data.AnonymousCall) {
                            QB.Phone.Notifications.Add("fas fa-phone", "Telefoon", "Je begon een anonieme oproep!");
                        }
...
QB.Phone.Functions.LoadContacts = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ...
            if (contact.name == '') contact.name = 'Dees zou ik niet hebben kunnen doen...'
        ...
        $("#total-contacts").text(TotalContacts+ " contact(en)");
    } else {
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
        ...
        setTimeout(function(){
            ...
            QB.Phone.Functions.ToggleApp("Telefoon", "none");
            QB.Phone.Functions.ToggleApp("messageapp", "block");
            QB.Phone.Data.currentApplication = "messageapp";

            $.post('https://qb-phone/GetmessageappChat', JSON.stringify({phone: ContactData.number}), function(chat){
            ...
            $(".messageapp-openedchat").css({left: 0+"vh"});
            $(".messageapp-chats").animate({left: 30+"vh"},100, function(){
                $(".messageapp-chats").css({"display":"none"});
            });
        }, 400)
    } else {
        QB.Phone.Notifications.Add("fa fa-phone-alt", "Telefoon", "Je kunt jezelf niet whatsapp..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact', function(e){
    ...
    if (CurrentEditContactData.name == '') CurrentEditContactData.name = 'Hmm, ik zou dit niet moeten kunnen doen...'
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName == '') ContactName = 'Hmm, ik zou dit niet moeten kunnen doen...'
    ...
    if (ContactName != "" && ContactNumber != "") {
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Edit Contact", "Vul alle velden in!");
    }
});
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Voeg contact toe", "Vul alle velden in!");
    }
});
        ...
        if (cData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-phone", "Telefoon", "Deze persoon is in een oproep!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Telefoon", "Deze persoon is niet beschikbaar!");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Telefoon", "U kunt uw eigen nummer niet noemen!");
        }
    });
}
...
IncomingCallAlert = function(CallData, Canceled, AnonymousCall) {
    if (!Canceled) {
        if (!QB.Phone.Data.CallActive) {
            ...
            QB.Phone.Animations.TopSlideUp('.'+QB.Phone.Data.currentApplication+"-app", 400, -160);
            setTimeout(function(){
                var Label = "Nieuwe oproep van "+CallData.name
                if (AnonymousCall) {
                    Label = "Nieuwe oproep van anonymuis"
                }
...
QB.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        ...
        if (cData.CallType == "incoming") {
            $(".phone-currentcall-title").html("Binnenkomend telefoongesprek");
        } else if (cData.CallType == "outgoing") {
            $(".phone-currentcall-title").html("Uitgaande oproep");
        } else if (cData.CallType == "ongoing") {
            $(".phone-currentcall-title").html("In oproep ("+cData.CallTime+")");
        }
