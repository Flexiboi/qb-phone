...
QB.Phone.Functions.SetupApplications = function(data) {
    ...
    $.each(data.applications, function(i, app){
        ...
        if ((!app.job || app.job === QB.Phone.Data.PlayerJob.name)) {
            $(applicationSlot).css({"background":'linear-gradient(to right, #'+app.color+' 0%, #'+LightenDarkenColor(app.color,-30)+' 100%)'});
            $(applicationSlot).css({"filter":'drop-shadow(0px 2px 2px rgba(14, 14, 14, 0.302)'});
            $(applicationSlot).css({"border":'4px outset rgba(255, 255, 255, 0.175)'});
            var icon = '<i class="ApplicationIcon '+app.icon+'" style="'+app.style+'"></i>';
            // if (app.app == "meos") {
            //     icon = '<img src="./img/politie.png" class="police-icon">';
            // }
            $(applicationSlot).html(icon+'<div class="app-unread-alerts">0</div><div class="app-title">'+app.tooltipText+'</div>');
...
$(document).on('click', '.phone-application', function(e){
    ...
    if (AppObject.length !== 0) {
        if (CanOpenApp) {
            if (QB.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "settings") {
                    ...
                    $("#mySerialNumber").text("DVN-" + QB.Phone.Data.PlayerData.metadata["phonedata"].SerialNumber);
                ...
                } else if (PressedApplication == "messageapp") {
                ...
                } else if (PressedApplication == "pings" || PressedApplication == "ping") {
        ...
        if (PressedApplication != null){
            QB.Phone.Notifications.Add("fas fa-exclamation-circle", "System", 'Deze app is in onderhoud. Kom later terug..')
        }
    }
});
...
$(document).on('click', '.phone-home-container', function(event){
    ...
    if (QB.Phone.Data.currentApplication === null) {
        ...
        if (QB.Phone.Data.currentApplication == "messageapp") {
...
QB.Phone.Functions.Close = function() {
    if (QB.Phone.Data.currentApplication == "messageapp") {
        setTimeout(function(){
            QB.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
            QB.Phone.Animations.TopSlideUp('.'+QB.Phone.Data.currentApplication+"-app", 400, -160);
            $(".messageapp-app").css({"display":"none"});
            QB.Phone.Functions.HeaderTextColor("white", 300);
    ...
    $.post('https://qb-phone/HasPhone', JSON.stringify({}), function(HasPhone){
        if (HasPhone) {
            ...
            if (QB.Phone.Notifications.Timeout == undefined || QB.Phone.Notifications.Timeout == null) {
                ...
                QB.Phone.Animations.TopSlideDown(".phone-notification-container", 350, 0);
                // if (icon !== "politie") {
                ...
                QB.Phone.Notifications.Timeout = setTimeout(function(){
                    QB.Phone.Animations.TopSlideUp(".phone-notification-container", 450, -8);
                    if (!QB.Phone.Data.IsOpen) {
...
QB.Phone.Functions.LoadPhoneData = function(data) {
    ...
    QB.Phone.Data.HasVpn = data.PhoneData.HasVpn;
    ...
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            ...
            case "UpdateMentionedTweets":
                QB.Phone.Notifications.LoadMentionedTweets(event.data.Tweets, event.data.HasVpn);
                break;
            case "UpdateBank":
            ...
            case "UpdateChat":
                if (QB.Phone.Data.currentApplication == "messageapp") {
                    if (OpenedChatData.number !== null && OpenedChatData.number == event.data.chatNumber) {
                        ...
                        QB.Phone.Functions.LoadmessageappChats(event.data.Chats);
                    }
                }
                break;
            case "UpdateHashtags":
                ...
                break;
            case "RefreshmessageappAlerts":
                QB.Phone.Functions.ReloadmessageappAlerts(event.data.Chats);
                break;
            ...
            case "UpdateCallTime":
                ...
                if (!QB.Phone.Data.IsOpen) {
                    if ($(".call-notifications").css("bottom") !== "52.1px") {
                        $(".call-notifications").css({"display":"block"});
                        $(".call-notifications").animate({bottom: 8+"vh"});
                    }
            ...
            case "CancelOngoingCall":
                $(".call-notifications").animate({bottom: -10+"vh"}, function(){
                    $(".call-notifications").css({"display":"none"});
                });
                QB.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
                setTimeout(function(){
            ...
            case "RefreshAdverts":
                if (QB.Phone.Data.currentApplication == "advert") {
                    QB.Phone.Notifications.LoadAdverts(event.data.Adverts);
                }
                break;
            case "UpdateTweets":
...
window.onkeydown = function(e){
    ...
    if ( e.target.nodeName == 'TEXTAREA' || e.target.nodeName == 'INPUT' || e.target.nodeName == 'DIV' ) {
        $.post('https://qb-phone/SetTypeState', JSON.stringify({IsTyping: true}), function(){});
    } else {
        $.post('https://qb-phone/SetTypeState', JSON.stringify({IsTyping: false}), function(){});
    }
};