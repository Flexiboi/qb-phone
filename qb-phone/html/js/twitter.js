...
$(document).on('click', '.twitter-header-tab', function(e){
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab == "twitter-home") {
            $.post('https://qb-phone/GetTweets', JSON.stringify({}), function(Tweets, HasVpn){
                QB.Phone.Data.HasVpn = HasVpn;
                QB.Phone.Notifications.LoadTweets(Tweets, HasVpn);
            });
        }
        ...
        $.post('https://qb-phone/GetMentionedTweets', JSON.stringify({}), function(MentionedTweets, HasVpn){
            QB.Phone.Data.HasVpn = HasVpn;
            QB.Phone.Notifications.LoadMentionedTweets(MentionedTweets, HasVpn)
        })
    }
});
...
$(document).on('click', '.twitter-new-tweet', function(e){
    ...
    if(QB.Phone.Data.HasVpn) {
        $("#send-tweet-vpn").css("display", "block");
        $(".twitter-new-tweet-button").css("width", '25%');
    } else {
        $("#send-tweet-vpn").css("display", "none");
        $(".twitter-new-tweet-button").css("width", '33%');
    }
});
...
QB.Phone.Notifications.LoadTweets = function(Tweets, HasVpn) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            if (clean == '') clean = 'Hmm, Error 404..?'
            var TwtMessage = QB.Phone.Functions.FormatTwitterMessage(clean);
            var Time = moment(Tweet.date).format('hh:mm');
            var Days = moment(Tweet.date).format('MM/DD/YYYY');
            ...
            if (Tweet.url == "") {
                ...
                if(!Tweet.vpntweet){
                    TweetElement = '<div class="twitter-tweet" data-twtcid="'+Tweet.citizenid+'" data-twtid ="'+Tweet.tweetId+'" data-twthandler="@' + TwitterHandle.replace(" ", "_") + '"><div class="tweet-reply"><i class="fas fa-reply"></i></div>' +
                        '<div class="tweet-tweeter">' + Tweet.firstName + ' ' + Tweet.lastName + '</div> <div class="tweet-handle">@' + TwitterHandle.replace(" ", "_") + '</div> <div class="tweet-time">OM: '+Time+'</div><div class="tweet-days">'+Days+'</div>' +
                    ...
                    TweetElement = '<div class="twitter-tweet-vpn" data-twtcid="'+Tweet.citizenid+'" data-twtid ="'+Tweet.tweetId+'" data-twthandler="@' + TwitterHandle.replace(" ", "_") + '"><div class="tweet-reply"><i class="fas fa-reply"></i></div>' +
                    '   <div class="tweet-tweeter">' + Tweet.firstName + ' ' + Tweet.lastName + '</div> <div class="tweet-handle">@' + TwitterHandle.replace(" ", "_") + '</div> <div class="tweet-time">OM: '+Time+'</div><div class="tweet-days">'+Days+'</div>' +
                        '<div class="tweet-message">' + TwtMessage + '</div>' +
                        '<div class="twt-img" style="top: 1vh;"><img src="' + PictureUrl + '" class="tweeter-image"></div>' +
                        '</div>';
                ...
                if(!Tweet.vpntweet){
                    TweetElement = '<div class="twitter-tweet" data-twthandler="@'+TwitterHandle.replace(" ", "_")+'"><div class="tweet-reply"><i class="fas fa-reply"></i></div>'+
                        '<div class="tweet-tweeter">' + Tweet.firstName + ' ' + Tweet.lastName + '</div> <div class="tweet-handle">@' + TwitterHandle.replace(" ", "_") + '</div> <div class="tweet-time">OM: '+Time+'</div><div class="tweet-days">'+Days+'</div>' +
                        '<div class="tweet-message">'+TwtMessage+'</div>'+
                        '<img class="image" src= ' + Tweet.url + ' style = "image-rendering: pixelated; filter: blur(3px); -webkit-filter: blur(3px); border-radius:4px; width: 70%; position:relative; z-index: 1; left:40px; margin:.6rem .5rem .6rem 1rem;height: auto; padding-bottom: 15px;">' +
                        '<div class="twt-img" style="top: 1vh;"><img src="'+PictureUrl+'" class="tweeter-image"></div>' +
                        '</div>';
                        $(".twitter-home-tab").append(TweetElement);
                } else if(HasVpn) {
                    TweetElement = '<div class="twitter-tweet-vpn" data-twthandler="@'+TwitterHandle.replace(" ", "_")+'"><div class="tweet-reply"><i class="fas fa-reply"></i></div>'+
                    '<div class="tweet-tweeter">' + Tweet.firstName + ' ' + Tweet.lastName + '</div> <div class="tweet-handle">@' + TwitterHandle.replace(" ", "_") + '</div> <div class="tweet-time">OM: '+Time+'</div><div class="tweet-days">'+Days+'</div>' +
                    '<div class="tweet-message">'+TwtMessage+'</div>'+
                    '<img class="image" src= ' + Tweet.url + ' style = "image-rendering: pixelated; filter: blur(3px); -webkit-filter: blur(3px); border-radius:4px; width: 70%; position:relative; z-index: 1; left:40px; margin:.6rem .5rem .6rem 1rem;height: auto; padding-bottom: 15px;">' +
                    '<div class="twt-img" style="top: 1vh;"><img src="'+PictureUrl+'" class="tweeter-image"></div>' +
...
QB.Phone.Notifications.LoadMentionedTweets = function(Tweets, HasVpn) {
    Tweets = Tweets.reverse();
    QB.Phone.Data.HasVpn = HasVpn;
    $('#tweet-new-url').val("");
    if (Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            var TweetElement = null;
            if(!hasvpn){
                TweetElement = '<div class="twitter-tweet" data-twtcid="'+Tweet.citizenid+'" data-twtid ="'+Tweet.tweetId+'" data-twthandler="@' + TwitterHandle.replace(" ", "_") + '"><div class="tweet-reply"><i class="fas fa-reply"></i></div>' +
                '<div class="tweet-tweeter">' + Tweet.firstName + ' ' + Tweet.lastName + ' </br><span>@' + TwitterHandle.replace(" ", "_") + ' <span class="tweet-time">OM: '+Time+'</span><span class="tweet-days">'+Days+'</span></span></div>' +
                '<div class="tweet-message">' + TwtMessage + '</div>' +
                '<div class="twt-img" style="top: 1vh;"><img src="' + PictureUrl + '" class="tweeter-image"></div>' +
                '</div>';
            } else {
...
$(document).on('click', '#send-tweet', function(e){
    ...
    if (TweetMessage != "") {
        ...
        $.post('https://qb-phone/PostNewTweet', JSON.stringify({
            ...
            url: imageURL,
            vpntweet: false,
        }), function(Tweets, HasVpn){
            QB.Phone.Data.HasVpn = HasVpn;
            QB.Phone.Notifications.LoadTweets(Tweets, false);
        });
...
$(document).on('click', '#send-tweet-vpn', function(e){
    e.preventDefault();
    var TweetMessage = $("#tweet-new-message").val();
    var imageURL = $('#tweet-new-url').val();
    if (TweetMessage != "") {
        ...
        $.post('https://qb-phone/PostNewTweet', JSON.stringify({
            ...
            QB.Phone.Data.HasVpn = HasVpn;
            QB.Phone.Notifications.LoadTweets(Tweets, true);
        });
        $.post('https://qb-phone/GetHashtags', JSON.stringify({}), function(Hashtags){
            QB.Phone.Notifications.LoadHashtags(Hashtags)
        })
...
QB.Phone.Notifications.LoadHashtagMessages = function(Tweets) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            var TweetElement ='<div class="twitter-tweet" data-twtcid="'+Tweet.citizenid+'" data-twtid ="'+Tweet.tweetId+'" data-twthandler="@' + TwitterHandle.replace(" ", "_") + '"><div class="tweet-reply"><i class="fas fa-reply"></i></div>' +
            '<div class="tweet-tweeter">' + Tweet.firstName + ' ' + Tweet.lastName + ' </br><span>@' + TwitterHandle.replace(" ", "_") + ' <span class="tweet-time">OM: '+Time+'</span><span class="tweet-days">'+Days+'</span></span></div>' +
            '<div class="tweet-message">' + TwtMessage + '</div>' +
            '<div class="twt-img" style="top: 1vh;"><img src="' + PictureUrl + '" class="tweeter-image"></div>' +
            '</div>';
