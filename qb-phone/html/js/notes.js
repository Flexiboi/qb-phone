...
$(document).on('click', '.notes-add', function(e) {
    ...
    $("#new-note-header-text").text('Notitie toevoegen')
...
$(document).on('click', '#new-note-submit', function(e) {
    ...
    if (note !== "") {
        ...
        if ( $("#new-note-header-text").text() === 'Notitie toevoegen' ) {
            $.post('https://qb-phone/PostNote', JSON.stringify({
                body: note,
                title: name,
                //date: date
            }));
        } else if ( $("#new-note-header-text").text() === 'Notitie bewerken') {
        ...
        QB.Phone.Notifications.Add("fas fa-comment", "Notes", "Je kunt geen lege notitie posten!", "#ff8f1a", 2000);
    }
});
...
$(document).on('click', '.note', function(e) {
    ...
    if ( Id ) { 
        ...
        $("#new-note-header-text").text('Notitie bewerken')
...
RefreshNotes = function(Notes) {
    ...
    if (Object.keys(Notes).length > 0) {
        ...
        $(".notes-list").html("");
        var element = '<div class="note"><span class="note-sender">Er zijn nog geen aantekeningen!</span></div>';
